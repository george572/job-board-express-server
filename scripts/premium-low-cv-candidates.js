/**
 * Find premium jobs with 0-1 CVs sent, get 3-4 good match candidates for each.
 * Logs job + candidate summaries.
 *
 * Usage:
 *   node scripts/premium-low-cv-candidates.js           # dry run (no send)
 *   node scripts/premium-low-cv-candidates.js --send    # send emails to all HRs
 *
 * Or hit GET /jobs/premium-low-cv-candidates (logs to server, returns JSON).
 */
  ...
  for (const m of qualified) {
    ...
    if (isNoCv) {
      ...
      try {
        ...
        console.warn(`  [skip no_cv_${nid}] ${e.message}`);
      }
    } else {
    ...
    list.length > 0
      ? list
          .map((u) => {
            ...
            if (url && url.trim() && url !== "—") lines.push(`CV ლინკი : ${url}`);
...
async function main() {
  ...
  const jobs = await db("jobs")
    ...
    .where((qb) => {
      qb.where("cvs_sent", 0).orWhere("cvs_sent", 1).orWhereNull("cvs_sent");
    })
    .select(
      "id",
      "jobName",
  ...
  console.log(`\n=== Premium jobs (0-1 CVs sent): ${jobs.length} found ===\n`);

  const results = [];
  for (const job of jobs) {
    console.log("─".repeat(70));
    console.log(
      `Job #${job.id}: ${job.jobName} @ ${job.companyName} (cvs_sent: ${job.cvs_sent || 0})`,
    );
    console.log(`  HR: ${job.company_email || "(none)"}`);

    const candidates = await getTopGoodCandidatesForJob(
      job,
      CANDIDATES_PER_JOB,
    );

    if (candidates.length === 0) {
      console.log("  No good/strong match candidates found.\n");
    ...
    candidates.forEach((c, i) => {
      console.log(
        `  ${i + 1}. ${c.name || "—"} (${c.verdict}) ${c.email || ""}`,
      );
      console.log(`     Summary: ${(c.ai_description || "—").slice(0, 150)}...`);
      if (c.cv_url) console.log(`     CV: ${c.cv_url}`);
      console.log("");
    });

    const jobLink = `${SITE_BASE_URL}/vakansia/${slugify(job.jobName)}-${job.id}`;
    const usersList = candidates.map(toHrEmailUser);
  ...
  if (doSend && marketingTransporter) {
    console.log("\n=== SENDING EMAILS TO ALL HRS ===\n");
    let sent = 0;
    for (const r of results) {
      if (r.candidates.length === 0 || !r.job.company_email) continue;
      try {
        await sendHrEmail(r.sendPayload);
        await db("jobs")
          .where("id", r.job.id)
          .increment("cvs_sent", r.candidates.length);
        console.log(`  ✓ Sent to ${r.job.company_email} (job #${r.job.id}: ${r.job.jobName}, ${r.candidates.length} candidates)`);
        sent++;
      } catch (err) {
        console.error(`  ✗ Failed job #${r.job.id}: ${err.message}`);
      }
    }
    console.log(`\nDone. Sent ${sent} email(s) to HRs.\n`);
  } else if (!doSend) {
    console.log("\n=== TO SEND TO ALL HRS ===\n");
    console.log("Run with --send to email all HRs:\n");
    console.log("  node scripts/premium-low-cv-candidates.js --send\n");
  } else if (!marketingTransporter) {
    console.log("\n⚠ MARKETING_MAIL_USER / MARKETING_MAIL_PASS not set. Cannot send.\n");
  }
